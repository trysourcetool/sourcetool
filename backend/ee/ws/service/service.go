...
type websocketServiceEE struct {
	*infra.Dependency
	*service.WebSocketServiceCE
}

func NewWebSocketServiceEE(d *infra.Dependency) *websocketServiceEE {
	return &websocketServiceEE{
		Dependency:         d,
		WebSocketServiceCE: service.NewWebSocketServiceCE(infra.NewDependency(d.Store, d.Mailer)),
	}
}

func (s *websocketServiceEE) InitializeHost(ctx context.Context, conn *gorillaws.Conn, instanceID string, msg *websocketv1.Message) (*hostinstance.HostInstance, error) {
	...
	groups, err := s.Store.Group().List(ctx, group.ByOrganizationID(apikey.OrganizationID), group.BySlugs(allGroupSlugs))
	if err != nil {
		return nil, err
	}
