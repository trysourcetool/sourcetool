	...
	for {
		...
		case <-ticker.C:
			if err := c.pingConnection(host.conn); err != nil {
				...
				if err := c.repo.HostInstance().Update(context.Background(), host.hostInstance); err != nil {
	...
	for {
		...
		case <-ticker.C:
			if err := c.pingConnection(client.conn); err != nil {
				...
				if err := c.repo.Session().Delete(context.Background(), client.session); err != nil {
...
func (c *connManager) SetConnectedClient(session *session.Session, conn *websocket.Conn) {
