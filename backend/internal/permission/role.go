...
func (c *Checker) AuthorizeOperation(ctx context.Context, o operation) error {
	...
	orgAccess, err := c.repo.User().GetOrganizationAccess(ctx, user.OrganizationAccessByUserID(currentUser.ID), user.OrganizationAccessByOrganizationID(currentOrg.ID))
