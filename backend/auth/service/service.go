...
func NewAuthServiceCE(d *infra.Dependency) *AuthServiceCE {
...
func (s *AuthServiceCE) RequestMagicLink(ctx context.Context, in dto.RequestMagicLinkInput) (*dto.RequestMagicLinkOutput, error) {
	...
	if config.Config.IsCloudEdition {
		...
		if subdomain != "" && subdomain != "auth" {
			...
			if exists {
				...
				u, err := s.Store.User().Get(ctx, user.ByEmail(in.Email))
				if err != nil {
					return nil, err
				}
	...
	if exists {
		...
		orgAccesses, err := s.Store.User().ListOrganizationAccesses(ctx, user.OrganizationAccessByUserID(u.ID))
		if err != nil {
			return nil, err
		}
		...
		if config.Config.IsCloudEdition && len(orgAccesses) > 1 {
			...
			for _, access := range orgAccesses {
				org, err := s.Store.Organization().Get(ctx, organization.ByID(access.OrganizationID))
				if err != nil {
					return nil, err
				}
			...
			if err := s.Mailer.User().SendMultipleOrganizationsMagicLinkEmail(ctx, &user.SendMultipleOrganizationsMagicLinkEmail{
				To:        in.Email,
				FirstName: firstName,
				Email:     in.Email,
				LoginURLs: loginURLs,
	...
	if err := s.Mailer.User().SendMagicLinkEmail(ctx, &user.SendMagicLinkEmail{
		To:        in.Email,
		FirstName: firstName,
		URL:       url,
	}); err != nil {
...
func (s *AuthServiceCE) AuthenticateWithMagicLink(ctx context.Context, in dto.AuthenticateWithMagicLinkInput) (*dto.AuthenticateWithMagicLinkOutput, error) {
	...
	// Get existing user
	u, err := s.Store.User().Get(ctx, user.ByEmail(c.Email))
	if err != nil {
		return nil, err
	}
	...
	if config.Config.IsCloudEdition {
		...
		orgAccess = orgAccesses[0]
		_, err = s.Store.Organization().Get(ctx, organization.ByID(orgAccess.OrganizationID))
		if err != nil {
			return nil, err
		}
	}
...
func (s *AuthServiceCE) RequestInvitationMagicLink(ctx context.Context, in dto.RequestInvitationMagicLinkInput) (*dto.RequestInvitationMagicLinkOutput, error) {
	...
	// Get invitation
	userInvitation, err := s.Store.User().GetInvitation(ctx, user.InvitationByEmail(c.Email))
	if err != nil {
		return nil, err
	}

	// Get organization
	invitedOrg, err := s.Store.Organization().Get(ctx, organization.ByID(userInvitation.OrganizationID))
	if err != nil {
		return nil, err
	}
	...
	if config.Config.IsCloudEdition {
		subdomain := ctxutil.Subdomain(ctx)
		hostOrg, err := s.Store.Organization().Get(ctx, organization.BySubdomain(subdomain))
		if err != nil {
			return nil, err
		}
	...
	// Send magic link email
	if err := s.Mailer.User().SendInvitationMagicLinkEmail(ctx, &user.SendInvitationMagicLinkEmail{
		To:        c.Email,
		URL:       url,
...
func (s *AuthServiceCE) AuthenticateWithGoogle(ctx context.Context, in dto.AuthenticateWithGoogleInput) (*dto.AuthenticateWithGoogleOutput, error) {
	...
	if !exists {
		...
		if stateClaims.Flow == jwt.GoogleAuthFlowInvitation {
			...
			userInvitation, err := s.Store.User().GetInvitation(ctx, user.InvitationByEmail(userInfo.email), user.InvitationByOrganizationID(stateClaims.InvitationOrgID))
			if err != nil {
	...
	// For existing users
	u, err := s.Store.User().Get(ctx, user.ByEmail(userInfo.email))
	if err != nil {
	...
	if stateClaims.Flow == jwt.GoogleAuthFlowInvitation {
		...
		invitedOrg, err := s.Store.Organization().Get(ctx, organization.ByID(stateClaims.InvitationOrgID))
		if err != nil {
		...
		if config.Config.IsCloudEdition {
			if len(orgAccesses) > 1 {
				...
				if hostSubdomain == "" {
					...
					// Send email with multiple organization links
					if err := s.Mailer.User().SendMultipleOrganizationsLoginEmail(ctx, &user.SendMultipleOrganizationsLoginEmail{
						To:        u.Email,
						FirstName: u.FirstName,
						Email:     u.Email,
						LoginURLs: loginURLs,
					...
					org, err = s.Store.Organization().Get(ctx, organization.BySubdomain(hostSubdomain))
					if err != nil {
						return nil, err
					}
...
func (s *AuthServiceCE) RegisterWithGoogle(ctx context.Context, in dto.RegisterWithGoogleInput) (*dto.RegisterWithGoogleOutput, error) {
	...
	err = s.Store.RunTransaction(func(tx infra.Transaction) error {
		...
		if claims.Flow == jwt.GoogleAuthFlowInvitation {
			invitedOrg, err := s.Store.Organization().Get(ctx, organization.ByID(claims.InvitationOrgID))
			if err != nil {
...
func (s *AuthServiceCE) RefreshToken(ctx context.Context, in dto.RefreshTokenInput) (*dto.RefreshTokenOutput, error) {
	...
	u, err := s.Store.User().Get(ctx, user.ByRefreshTokenHash(hashedRefreshToken))
	if err != nil {
		return nil, errdefs.ErrUnauthenticated(err)
	}
...
func (s *AuthServiceCE) createPersonalAPIKey(ctx context.Context, tx infra.Transaction, u *user.User, org *organization.Organization) error {
	devEnv, err := s.Store.Environment().Get(ctx, environment.ByOrganizationID(org.ID), environment.BySlug(environment.EnvironmentSlugDevelopment))
	if err != nil {
		return err
	}
...
func (s *AuthServiceCE) createTokens(userID uuid.UUID, expiration time.Duration) (token, xsrfToken, plainRefreshToken, hashedRefreshToken string, expiresAt time.Time, err error) {
	now := time.Now()
...
func (s *AuthServiceCE) resolveOrganizationBySubdomain(ctx context.Context, u *user.User, subdomain string) (*organization.Organization, *user.UserOrganizationAccess, error) {
	if subdomain == "" {
...
func (s *AuthServiceCE) validateSelfHostedOrganization(ctx context.Context) error {
	if !config.Config.IsCloudEdition {
...
func (s *AuthServiceCE) createInitialOrganizationForSelfHosted(ctx context.Context, tx infra.Transaction, u *user.User) error {
	if config.Config.IsCloudEdition {
		return nil
	}
...
func (s *AuthServiceCE) getUserOrganizationInfo(ctx context.Context) (*organization.Organization, *user.UserOrganizationAccess, error) {
...
func (s *AuthServiceCE) getOrganizationBySubdomain(ctx context.Context, u *user.User, subdomain string) (*organization.Organization, *user.UserOrganizationAccess, error) {
	// Get organization by subdomain
	org, err := s.Store.Organization().Get(ctx, organization.BySubdomain(subdomain))
	if err != nil {
		return nil, nil, err
...
func (s *AuthServiceCE) getOrganizationInfo(ctx context.Context, u *user.User) (*organization.Organization, *user.UserOrganizationAccess, error) {
	if u == nil {
...
func (s *AuthServiceCE) getDefaultOrganizationForUser(ctx context.Context, u *user.User) (*organization.Organization, *user.UserOrganizationAccess, error) {
